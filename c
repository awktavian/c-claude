...
#   c '**/*.py' 'analyze' -o results/             # save to results/file.txt
#   c '**/*.py' -i template.txt -o results/ -p 8  # instruction from file
#   c '**/*.py' 'add tests' -a                    # agentic (auto-accept edits)
# Flags:
...
case $1 in help|-h|--help)sed -n 2,16p "$0";exit;;esac
[ $# -lt 1 ]&&echo "Usage: c GLOB INSTRUCTION [-flags...] OR c GLOB -i FILE [-flags...]">&2&&exit 1
...
[ $V = 1 ]&&echo "Files: $(echo "$F"|wc -l|tr -d ' '), Parallelism: $P">&2
